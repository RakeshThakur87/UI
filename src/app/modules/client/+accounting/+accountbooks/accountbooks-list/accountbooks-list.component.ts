...
@Component({
  selector: 'accountbook-list',
  templateUrl: './accountbook-list.html',
  styleUrls: ['./accountbook-list.css'],
  providers: [AccountingService,PagerService]
})
export class AccountbookComponent implements OnInit {
  ...
  constructor(public apiService: AccountingService, private router: Router,public pagerService: PagerService) {
  ...
  getListData(currentPageNo) { 
    ...
    this.apiService.GetAccountPagingList(this.elements).subscribe(res => {
